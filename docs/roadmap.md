...
## Phase 1: MVP（現行計画） 🔧 実装中

> 詳細は [specification.md](./specification.md) を参照
> 実装進捗は [development-status.md](./development-status.md) を参照（チームレビュー決定事項含む）
...
- ✅ 辞書機能 MVP（Lv.1-2 範囲、42エントリ、カテゴリ別/アルファベット順表示、サイドバー検索）→ 詳細は [dictionary_docs/](./dictionary_docs/) 参照
- ✅ 演習コンテンツ Lv.0-1 MVP（環境構築4演習 + Docker基礎操作6演習）→ 詳細は [practice_docs/](./practice_docs/) 参照
- ⬜ **進捗ダッシュボード完成**（Coming Soonプレースホルダー → 実装）← 🔴 チームレビューで最優先決定
- ⬜ **Vercelへのデプロイ** ← 🔴 MVP公開の前提
- ⬜ **セキュリティヘッダー設定**（next.config.js）← 🔴 デプロイ前に実施
- ⬜ **プライバシーポリシーページ** ← 🔴 デプロイ前に法務要件として必須
- ⬜ **XPトースト・レベルアップ演出** ← 🟠 近く対応
- ⬜ **レスポンシブデザイン / モバイル対応** ← 🟠 近く対応（チームレビューで優先度アップ）
- ⬜ **演習コンテンツ Lv.2-3 追加** ← 🟠 近く対応（チームレビューで確認）
- ⬜ Lv.4-5 コンテンツ品質向上（工事中）
- ⬜ 辞書「コンテナ一般の概念」グループの解説を汎用化
...
| フェーズ | 対策 | 詳細 |
|---------|------|------|
| **MVP** | セキュリティヘッダー | `next.config.js` に X-Frame-Options / X-Content-Type-Options / Referrer-Policy を追加 ← 🔴 デプロイ前に実施（チームレビュー決定） |
...
| 要件 | 対応 |
|------|------|
| **個人情報保護法** | プライバシーポリシー / 利用規約 / データ削除権 ← 🔴 Vercelデプロイ前に必須（アクセスログ収集が始まる時点で適用対象） |
| **GDPR**（海外展開時） | Cookie同意 / データポータビリティ / 忘れられる権利 |
| **PCI DSS** | Stripe使用により自社でのカード情報非保持で対応 |
| **特定商取引法** | 有料プラン提供時の表記義務 ← 🟡 課金導入前に必須（チームレビュー決定） |
| **「Docker」商標** | Docker, Inc.の登録商標。商業利用ガイドラインを確認し、「Docker Quest」名称の問題がないかを調査 ← 🟡 中期対応（チームレビュー決定） |
