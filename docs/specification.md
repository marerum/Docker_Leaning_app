...
**対象**: 自分のPCで実際に Docker を操作したい学習者

- メインカリキュラムの**実践パート**として位置づけ
- 環境構築 → Docker基礎 → Compose → クラウドデプロイまで全9レベル・36演習
- 詳細は [practice_docs/](./practice_docs/) を参照
...
- 詳細は [dictionary_docs/](./dictionary_docs/) を参照
...
> [!NOTE]
> チャプター番号は全体を通した通し番号（1〜22）。全5レベル構成。

### レベル1: Docker 基礎（初級者向け）— 全6章
...
| 3 | コンテナを操る | アプリのライフサイクル管理 | 起動・停止・削除 | `docker run/stop/rm/ps` |
| 4 | コンテナの中に入ろう | デバッグのために中に入る必要性 | 対話モード、exec | `docker run -it`, `docker exec` |
| 5 | ポートを開こう | コンテナを外部に繋ぐことでWebサービスとして公開 | ポートマッピング | `docker run -p` + ブラウザ確認 |
| 6 | データを永続化 | コンテナは消えたらデータも消える。永続化の方法 | ボリューム、バインドマウント | `docker volume`, `-v` |

### レベル2: Dockerfile & イメージ（初〜中級者向け）— 全3章

| # | 章タイトル | イントロで伝えること | 学習内容 | ハンズオン |
|---|-----------|-------------------|---------|-----------| 
| 7 | Dockerfileを書こう | 「環境構築を自動化する設計図」としてのDockerfile | FROM, RUN, COPY, CMD | Dockerfile作成 → `docker build` |
| 8 | レイヤーを理解する | なぜビルドが遅い/速いが変わるか | レイヤーキャッシュ、.dockerignore | `docker history`, ビルド最適化 |
| 9 | マルチステージビルド | 本番イメージを軽量に保つ理由 | ビルド用/実行用の分離 | サイズ比較 |

### レベル3: Docker Compose（中級者向け）— 全6章

| # | 章タイトル | イントロで伝えること | 学習内容 | ハンズオン |
|---|-----------|-------------------|---------|-----------| 
| 10 | Composeの世界へ | 手動管理の限界とComposeの価値 | YAMLの基本、docker-compose.yml | `docker compose up/down` |
| 11 | 複数サービスを連携 | マイクロサービス的発想 | サービス間通信、depends_on | Web + DB構成 |
| 12 | 環境変数と設定 | 設定の外部化がなぜ重要か | .env、environment、12-Factor | 設定の外部化 |
| 13 | ヘルスチェックとログ | 起動≠正常動作。障害検知の基本 | healthcheck, docker compose logs | ログ管理 |
| 14 | 開発ワークフロー実践 | 開発と本番の設定分離 | bind mount, compose.override.yml | ホットリロード環境 |
| 15 | Compose 実践プロジェクト | Lv.3の知識統合 | Web+DB+Cache 3サービス構成 | 総合演習 |

### レベル4: 実践・運用スキル（中〜上級者向け）— 全4章

| # | 章タイトル | イントロで伝えること | 学習内容 | ハンズオン |
|---|-----------|-------------------|---------|-----------| 
| 16 | デバッグとトラブルシュート | デバッグスキルは最も実用的 | docker logs, inspect, stats | エラー調査フロー |
| 17 | Docker ネットワーク | 暗黙のネットワークを明確化 | bridge/host/none, カスタムNW | ネットワーク分離 |
| 18 | イメージ管理と Docker Hub | チームでイメージを共有 | docker tag/push/pull, 命名規則 | Docker Hub push |
| 19 | セキュリティの基本 | root実行のリスクと対策 | USER命令, イメージスキャン, secrets | docker scout |

### レベル5: CI/CD・チーム開発（上級者向け）— 全3章

| # | 章タイトル | イントロで伝えること | 学習内容 | ハンズオン |
|---|-----------|-------------------|---------|-----------| 
| 20 | CI/CD と Docker | 自動化で品質と効率を向上 | GitHub Actions, build→test→push | CI/CDパイプライン |
| 21 | 本番デプロイ戦略 | クラウドサービスの選択肢 | ECS/Container Apps/Cloud Run | compose.prod.yml |
| 22 | Docker 総まとめ | 全知識の統合 | 全Level横断フロー | 総合チャレンジ |
...
- ✅ ガイドブック（レベル1-5: 全22章、各章にイントロ付き）
- ✅ シミュレーションターミナル（基本〜CI/CDコマンド対応）
...
### ユーザーテスト
- Docker未経験者に全22章を通しで実施、理解度・楽しさをヒアリング
...
| MVP範囲 | レベル1-5（Docker基礎 → イメージ → Compose → 実践・運用 → CI/CD）全22章 |
